...
class Txt2ImgGenerator:
    ...
    def generate_images(self):
        total_images = len(self.prompts[:NUM_IMAGES])
        batches = list(self.divide_chunks(self.prompts[:NUM_IMAGES], self.images_ids[:NUM_IMAGES], self.batch_size))
        ...
        for index, (batch_prompts, batch_image_ids) in enumerate(batches):
            ...
            with autocast(enabled=True):
                generated_images = self.model(batch_prompts, negative_prompt=[NEGATIVE_PROMPTS], num_inference_steps=self.num_inference_steps)["images"]
        ...
        result_df = pd.merge(input_df[:NUM_IMAGES], generated_info_df, on='id', how='left')
