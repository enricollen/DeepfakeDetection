...
class ImagesDataset(Dataset):
    def __init__(self, data_path, csv_path, size, tokenizer, set, modal_mode):
        ...
        self.fakenews = self.df['fakenews_text'].tolist()
    ...
    def __getitem__(self, idx):
        ...
        if image is not None:
            ...
            if self.set == "test":
                return image_name, image, caption_input_ids, attention_mask, label, fakenews
            else:
                return image, caption_input_ids, attention_mask, label, fakenews
        else:
