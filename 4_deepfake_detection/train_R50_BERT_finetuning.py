...
from images_dataset_R50_BERT_finetuning import ImagesDataset
from MLP import MLP
...
class BalancedSamplerNEW(torch.utils.data.sampler.Sampler):
    def __init__(self, dataset, batch_size, minority_label=0, majority_ratio=0.7):
        self.dataset = dataset
        self.batch_size = batch_size
        self.original_minority_indices = set()
        self.original_majority_indices = set()
        self.minority_label = minority_label
        self.majority_label = 1 - minority_label
        self.majority_ratio = majority_ratio
        self.minority_ratio = 1 - majority_ratio
        self.setup_indices()

    def setup_indices(self):
        label_to_indices = {0: [], 1: []}
        for i in range(len(self.dataset)):
            label = self.dataset.fakenews[i] 
            label_to_indices[label].append(i)
        self.original_minority_indices = set(label_to_indices[self.minority_label])
        self.original_majority_indices = set(label_to_indices[self.majority_label])
        self.reset_indices()

    def reset_indices(self):
        self.remaining_minority_indices = set(self.original_minority_indices)
        self.remaining_majority_indices = set(self.original_majority_indices)
        self.minority_per_batch = int(self.batch_size * self.minority_ratio)
        self.majority_per_batch = self.batch_size - self.minority_per_batch

    def __iter__(self):
        self.reset_indices()  # Reset indices at the start of each iteration over the sampler
        while len(self.remaining_minority_indices) >= self.minority_per_batch:
            batch_indices = set(random.sample(self.remaining_minority_indices, self.minority_per_batch))
            self.remaining_minority_indices -= batch_indices

            if len(self.remaining_majority_indices) >= self.majority_per_batch:
                majority_indices = set(random.sample(self.remaining_majority_indices, self.majority_per_batch))
                self.remaining_majority_indices -= majority_indices
                batch_indices.update(majority_indices)
            else:
                # If not enough majority samples, use what's left
                batch_indices.update(self.remaining_majority_indices)
                self.remaining_majority_indices.clear()

            yield from list(batch_indices)

    def __len__(self):
        return (len(self.original_minority_indices) // self.minority_per_batch) * self.batch_size
...
if __name__ == '__main__':
    ...
    train_sampler = BalancedSamplerNEW(train_dataset, BATCH_SIZE) 
    ...
    classifier.load_state_dict(torch.load("/home/enriconello/DeepFakeDetection/Thesis/5_biased_detection/comparison/train_1_multimodal/RN50_BERT/run3/best_multimodal_classifier.pth"))
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_index, (images, input_ids, attention_masks, labels, fakenews) in progress_bar:
