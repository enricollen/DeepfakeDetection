...
def load_model(best_model_path, model_weights):
    print("Loading model...")

    resnet50 = models.resnet50(pretrained=False)
    resnet50.fc = torch.nn.Linear(2048, 1) 
    resnet50.load_state_dict(torch.load(model_weights, map_location="cpu"))
...
if __name__ == "__main__":
    resnet50 = load_model(BEST_MODEL_PATH, '/home/enriconello/DeepFakeDetection/resnet50_image_only_stablediffusion_wiki_90')  

    test_dataset = ImagesDataset(TEST_DATA_PATH, TEST_CSV_PATH, IMAGE_SIZE, None, set="test", modal_mode=0)  
    ...
    for batch_index, (image_names, images, _, _, labels) in tqdm(enumerate(test_loader), total=len(test_loader), desc="Inference"):
        images = images.to(device)
        labels = labels.float().unsqueeze(1).to(device)

        with torch.no_grad():
            image_features = resnet50(images)
            predicted_test = torch.round(torch.sigmoid(image_features)).cpu().numpy()
